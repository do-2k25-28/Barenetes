...
/// A kubectl-style YAML manifest describing a pod, as written by the user.
    ...
    fn try_from(manifest: PodManifest) -> Result<Self, CliError> {
        if manifest.kind != "Pod" {
            return Err(CliError::InvalidUsage(format!(
                "unsupported manifest kind \"{}\", expected \"Pod\"",
                manifest.kind
            )));
        }
        if manifest.spec.containers.is_empty() {
            return Err(CliError::InvalidUsage(
                "manifest spec must include at least one container".to_string(),
            ));
        }
...
fn port_from_manifest(manifest: PortManifest, container_name: &str) -> Result<Port, CliError> {
    let protocol = match manifest.protocol.to_ascii_uppercase().as_str() {
        ...
        other => {
            return Err(CliError::InvalidUsage(format!(
                "unsupported protocol \"{other}\" on container \"{container_name}\", expected \"TCP\" or \"UDP\""
            )));
        }
    };
